...
LargeIntegerOTE* __fastcall LargeInteger::liNewSigned(intptr_t value)
{
	LargeIntegerOTE* oteR = reinterpret_cast<LargeIntegerOTE*>(Interpreter::NewIntPtr(value, Pointers.ClassLargeInteger));
	oteR->beImmutable();
	return oteR;
}
...
{
	// The limbs are always 32-bit words, even in the 64-bit VM
...
// The answer is not necessarily in the most reduced form possible (i.e. even if the 
// value would fit in a SmallInteger, this routine will still answer a LargeInteger).
LargeIntegerOTE* __fastcall LargeInteger::liNewUnsigned(uintptr_t value)
{
#ifdef _M_IX86
	...
	if (static_cast<int32_t>(value) < 0)
	{
		...
		oteLarge = reinterpret_cast<LargeIntegerOTE*>(Interpreter::NewIntPtr(value, Pointers.ClassLargeInteger));

	oteLarge->beImmutable();
	return oteLarge;
#else
	ULARGE_INTEGER* pValue = reinterpret_cast<ULARGE_INTEGER*>(&value);
...
Oop __stdcall Integer::NewUnsigned(uint64_t ullValue)
{
...
// Answer a new Integer instantiated from the 64-bit integer argument.
// The answer is in the most reduced form possible (i.e. it could even be a SmallInteger
Oop __stdcall Integer::NewSigned(int64_t value)
{
	...
	{
		// Only 32-bits needed
		oopAnswer = NewSigned(static_cast<int32_t>(lowPart));
	}
	else
	{
		// Full 64-bits needed
...
LargeIntegerOTE* __stdcall liLeftShift(LargeIntegerOTE* oteLI, unsigned shift)
{
	...
	size_t size = oteLI->getWordSize();
	const size_t resultSize = size + words + 1;
...
Oop __stdcall liRightShift(LargeIntegerOTE* oteLI, unsigned shift)
{
	...
	unsigned bits = shift & 31;
	unsigned words = shift >> 5;

	LargeInteger* li = oteLI->m_location;

	const size_t size = oteLI->getWordSize();
	ASSERT(size > 0);
	const ptrdiff_t resultSize = size - words;
	if (resultSize <= 0)
	...
	else
	{
		...
		uint32_t carry;
		for (intptr_t i=resultSize-2;i>=0;i--)
		{
			carry = digit << carryShift;
			const size_t j = i+words;
			ASSERT(j < size);
			digit = li->m_digits[j];
...
Oop __stdcall negateIntermediateResult(Oop oopInteger)
{
	if (ObjectMemoryIsIntegerObject(oopInteger))
	{
		...
		return Integer::NewSigned(-intValue);
	}
	else
	{
...
//
LargeIntegerOTE* LargeInteger::Add(const LargeIntegerOTE* oteLI, const int32_t operand)
{
	ASSERT(operand != 0);
...
LargeIntegerOTE* LargeInteger::Sub(const LargeIntegerOTE* oteLI, int32_t operand)
{
	const uint32_t* const digits = oteLI->m_location->m_digits;
...
//
Oop LargeInteger::Mul(const LargeIntegerOTE* oteInner, int32_t outerDigit)
{
	ASSERT(outerDigit != 0);
...
liDiv_t LargeInteger::Divide(const LargeIntegerOTE* oteU, int32_t v)
{
	...
	if (qSize == 1)
	{
		...
		Oop oopQuo = LargeInteger::NewSigned(quot);
		_ASSERTE(ObjectMemoryIsIntegerValue(rem));
	...
	else if (qSize == 2)
	{
		...
		return { LargeInteger::NewSigned(quo), oopRem };
	}
	else
	{
...
liDiv_t __stdcall liDivUnsigned(LargeIntegerOTE* oteEwe, LargeIntegerOTE* oteVee)
{
	...
	*/
	...
	for (size_t k=1;k<=static_cast<size_t>(m);k++)
	{
		...
		ASSERT(j >= 1 && static_cast<size_t>(j) <= eweSize);
...
/*
// Private - Answer the result of multiplying the receiver by the argument, anInteger
static LargeIntegerOTE* liMul32(uint32_t *const outer, size_t outerSize, uint32_t *const inner, size_t innerSize)
{
	...
	size_t productSize = innerSize + outerSize;
	LargeIntegerOTE* productPointer = LargeInteger::NewWithLimbs(productSize);
	LargeInteger* product = productPointer->m_location;
	...
	{
		// Result is negative
		for (size_t i=k;i<productSize;i++)
