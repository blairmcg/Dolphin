...
Oop* __fastcall Interpreter::primitiveValueWithArgs(Oop* const bp, uintptr_t)
{
	...
	*(bp-1) = closureReceiver;
	...
	const uint8_t extraTemps = block->stackTempsCount();
	{
	...
	const uint8_t envTemps = block->envTempsCount();
	if (envTemps > 0)
	{
...
Oop* __fastcall Interpreter::primitivePerform(Oop* const sp, primargcount_t argCount)
{
	...
	const uint8_t methodArgCount = method->m_header.argumentCount;
...
Oop* __fastcall Interpreter::primitivePerformWithArgs(Oop* const sp, primargcount_t)
{
	...
	const size_t argCount = argumentArray->pointersSize();
