...
public:
	...
	static constexpr size_t FirstBuiltInIdx = 1;
...
private:
	...
	static OTE** m_pZct;
	static intptr_t m_nZctEntries;		// Current no. of Zct entries
	static intptr_t m_nZctHighWater;	// High water mark at which ZCT reconciled
...
#define SizeOfPointers(N) (((N)+ObjectHeaderSize)*sizeof(Oop))
...
{
	unsigned nPool = (_ROUND2(objectSize, PoolGranularity) - MinObjectSize) / PoolGranularity;
	ASSERT(nPool < MaxPools);
	ASSERT(static_cast<uintptr_t>(nPool) * PoolGranularity + (uintptr_t)MinObjectSize >= objectSize);
