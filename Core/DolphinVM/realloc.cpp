...
inline POBJECT ObjectMemory::reallocChunk(POBJECT pChunk, size_t newChunkSize)
{
		...
		else
		{
			for (int i=0;i<NumPools;i++)
				ASSERT(!m_pools[i].isMyChunk(reinterpret_cast<uintptr_t*>(pChunk)));
		}
	#endif
...
POBJECT ObjectMemory::resizeVirtual(OTE* ote, size_t newByteSize)
{
	...
	else if (newTotalByteSize < currentTotalByteSize)
	{
		...
		uintptr_t* pCeiling = reinterpret_cast<uintptr_t*>(reinterpret_cast<BYTE*>(pBase) + newTotalByteSize);
...
VariantByteObject* ObjectMemory::resize(BytesOTE* ote, size_t newByteSize)
{
	...
	size_t totalByteSize = newByteSize + SizeOfPointers(0);	// Add header size

	VariantByteObject* pByteObj = ote->m_location;
	size_t oldByteSize = ote->getSize();
...
VariantObject* ObjectMemory::resize(PointersOTE* ote, size_t newPointers, bool bRefCount)
{
	...
	const size_t oldPointers = ote->pointersSize();
	...
	if (bRefCount)
	{
		for (size_t i=newPointers;i<oldPointers;i++)
	...
	if (pObj)
	{
		...
		for (size_t i=oldPointers; i<newPointers; i++)
			pObj->m_fields[i] = nil;
	}
