...
bool ObjectMemory::IsInZct(OTE* ote)
{
	...
	for (ptrdiff_t i = 0; i < m_nZctEntries; i++)
...
Oop* ObjectMemory::ReconcileZct()
{
	...
	const ptrdiff_t nOldZctEntries = m_nZctEntries;
#endif
...
OTE* __fastcall ObjectMemory::recursiveFree(OTE* rootOTE)
{
	...
	if (!rootOTE->isFinalizable())
	{
		...
		if (rootOTE->isPointers())
		{
			...
			const size_t lastPointer = rootOTE->getWordSize();
...
void ObjectMemory::PopulateZct(Oop* const sp)
{
	...
	else if ((m_nZctHighWater > static_cast<ptrdiff_t>(ZctMinSize)) && (m_nZctEntries < (m_nZctHighWater >> 2)))
	{
